...
def create_meal(request):
    ...
    if result["success"]:
        ...
        return Response(
            {"success": False, "message": result.get("error", "Failed to create meal")},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def get_meal_details(request, meal_id):
    ...
    if result["success"]:
        return Response({"success": True, "data": result["meal"]})
    else:
        return Response(
            {"success": False, "message": result.get("error", "Meal not found")},
...
def add_food_to_meal(request, meal_id):
    ...
    service = MealsService()
    result = service.add_food_to_meal(
        meal_id, request.user.id, food_id, quantity, food_data
    )
...
def get_user_meals(request):
    ...
    if user_timezone_offset_header:
        try:
            ...
            logger.warning(
                f"Invalid timezone offset in header: {user_timezone_offset_header}"
            )
    ...
    if result["success"]:
        ...
        return Response(
            {"success": False, "message": result.get("error", "Failed to get meals")},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def delete_meal(request, meal_id):
    ...
    if result["success"]:
        ...
        return Response(
            {"success": False, "message": result.get("error", "Failed to delete meal")},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def get_daily_summary(request):
    ...
    if result["success"]:
        return Response({"success": True, "data": result["summary"]})
    else:
        return Response(
            {
                "success": False,
...
def get_nutrition_stats(request):
    ...
    if result["success"]:
        return Response({"success": True, "data": result["stats"]})
    else:
        return Response(
            {
                "success": False,
...
def record_weight(request):
    ...
    if result["success"]:
        return Response(
            {
                "success": True,
                "data": {"date": result["date"], "weight": result["weight"]},
...
def update_meal(request, meal_id):
    ...
    try:
        ...
        return Response(
            {"success": False, "message": f"Failed to update meal: {str(e)}"},
...
def get_meal_statistics(request):
    ...
    if start_datetime_utc and end_datetime_utc:
        # 使用 UTC 时间范围
        try:
            filters["start_datetime_utc"] = datetime.fromisoformat(
                start_datetime_utc.replace("Z", "+00:00")
            )
            filters["end_datetime_utc"] = datetime.fromisoformat(
                end_datetime_utc.replace("Z", "+00:00")
            )
        except ValueError:
            return Response(
                {
    ...
    if result["success"]:
        return Response({"success": True, "data": result["statistics"]})
    else:
        return Response(
            {
                "success": False,
...
def get_meal_comparison(request):
    ...
    try:
        ...
        if result["success"]:
            return Response({"success": True, "data": result["comparison"]})
        else:
            return Response(
                {
                    "success": False,
        ...
        return Response(
            {"success": False, "message": f"Failed to get meal comparison: {str(e)}"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
