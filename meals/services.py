...
class MealsService:
    ...
    def _parse_timezone_filters(self, filters: Dict[str, Any]) -> Q:
        ...
        if start_datetime_utc or end_datetime_utc:
            ...
            logger.debug(
                f"Using UTC datetime filtering: {start_datetime_utc} to {end_datetime_utc}"
            )
            return query
        ...
        if filters.get("date"):
            ...
            try:
                ...
                logger.info(
                    f"Retrieved {Meal.objects.filter(query).count()} meals using legacy date filtering"
                )
        ...
        if filters.get("start_date"):
            try:
                ...
                start_utc = datetime.strptime(
                    f"{filters['start_date']} 00:00:00", "%Y-%m-%d %H:%M:%S"
                ).replace(tzinfo=pytz.UTC)
                query &= Q(date__gte=start_utc)
            except ValueError as e:
                logger.error(
                    f"Invalid start_date format: {filters['start_date']} - {e}"
                )
        ...
        if filters.get("end_date"):
            try:
                ...
                end_utc = datetime.strptime(
                    f"{filters['end_date']} 23:59:59", "%Y-%m-%d %H:%M:%S"
                ).replace(tzinfo=pytz.UTC)
        ...
        if filters.get("start_date") or filters.get("end_date") or filters.get("date"):
            logger.debug(
                f"Using legacy date filtering: {filters.get('start_date')} to {filters.get('end_date')}"
            )
    ...
    def create_meal(self, user_id: int, meal_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
                ...
                if meal_data.get("foods"):
                    for food_item in meal_data["foods"]:
                        ...
                        try:
                            ...
                            if not food:
                                logger.error(
                                    f"Food with ID {food_item['food_id']} not found and could not be created from USDA"
                                )
                                continue
            ...
            return {"success": False, "error": "One or more foods not found"}
        except Exception as e:
    ...
    def add_food_to_meal(
        ...
        try:
            with transaction.atomic():
                ...
                # Create meal food
                meal_food = MealFood.objects.create(
                    meal=meal, food=food, quantity=quantity
                )
    ...
    def update_meal_food(
        self, meal_food_id: int, user_id: int, quantity: Decimal
    ) -> Dict[str, Any]:
        ...
        try:
            with transaction.atomic():
                # Get meal food
                meal_food = MealFood.objects.get(id=meal_food_id, meal__user_id=user_id)
            ...
            return {"success": False, "error": "Meal food not found"}
        except Exception as e:
    ...
    def get_user_meals(self, user_id: int, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            has_datetime_filters = filters.get("start_datetime_utc") or filters.get(
                "end_datetime_utc"
            )
            ...
            logger.info(
                f"Retrieved {len(results)} meals using {filter_method} filtering"
            )
        ...
        try:
            ...
            return {
                ...
                "summary": {
                    ...
                    "weight_recorded": (
                        float(daily_summary.weight_recorded)
                        if daily_summary.weight_recorded
                        else None
                    ),
                ...
                daily_data.append(
                    {
                        ...
                        "weight": (
                            float(summary.weight_recorded)
                            if summary.weight_recorded
                            else None
                        ),
                    }
                )
    ...
    def record_weight(
        self, user_id: int, date: datetime.date, weight: Decimal
    ) -> Dict[str, Any]:
    ...
    def get_meal_statistics(
        self, user_id: int, date: datetime.date, meal_type: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            top_foods = sorted(
                food_stats.items(), key=lambda x: x[1]["total_calories"], reverse=True
            )[:10]
    ...
    def get_meal_statistics_with_filters(
        ...
        try:
            ...
            if filters.get("date"):
                date_info["date"] = (
                    filters["date"].isoformat()
                    if hasattr(filters["date"], "isoformat")
                    else str(filters["date"])
                )
            elif filters.get("start_datetime_utc") and filters.get("end_datetime_utc"):
                date_info["start_datetime_utc"] = filters[
                    "start_datetime_utc"
                ].isoformat()
        ...
        try:
            ...
            differences = {
                ...
                "total_calories": round(
                    summary2["total_calories"] - summary1["total_calories"], 2
                ),
                "total_protein": round(
                    summary2["total_protein"] - summary1["total_protein"], 2
                ),
                "total_fat": round(summary2["total_fat"] - summary1["total_fat"], 2),
                "total_carbs": round(
                    summary2["total_carbs"] - summary1["total_carbs"], 2
                ),
            }
            ...
            for key in ["total_calories", "total_protein", "total_fat", "total_carbs"]:
                if summary1[key] > 0:
                    percent_changes[key] = round(
                        (differences[key] / summary1[key]) * 100, 2
                    )
                else:
    ...
    def _create_usda_food_if_needed(
        self, food_item: Dict[str, Any], user_id: int
    ) -> Optional[Food]:
        """
        ...
        try:
            ...
            if existing_food:
                logger.info(
                    f"Found existing USDA food with FDC ID {fdc_id}: {existing_food.name}"
                )
            ...
            if not result["success"]:
                logger.error(
                    f"Failed to get USDA nutrition data for FDC ID {fdc_id}: {result.get('error')}"
                )
                return None
            ...
            food = Food.objects.create(
                name=nutrition_data.get(
                    "description", food_item.get("name", f"USDA Food {fdc_id}")
                ),
            ...
            logger.info(
                f"Successfully created USDA food: {food.name} (FDC ID: {fdc_id})"
            )
            return food

        except Exception as e:
            logger.error(
                f"Failed to create USDA food from food_item {food_item}: {str(e)}"
            )
            return None
