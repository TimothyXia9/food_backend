...
class MealAdmin(admin.ModelAdmin):
	...
	fieldsets = (
		('基本信息', {
			'fields': ('user', 'date', 'meal_type', 'name')
		}),
		('详细信息', {
			'fields': ('notes',)
		}),
		('营养统计', {
			'fields': ('get_total_calories', 'get_total_protein', 'get_total_fat', 'get_total_carbs'),
			'classes': ('collapse',)
		}),
		('时间戳', {
	...
	def get_queryset(self, request):
		"""优化查询性能"""
		return super().get_queryset(request).select_related('user').prefetch_related('meal_foods')
...
class MealFoodAdmin(admin.ModelAdmin):
	...
	def get_queryset(self, request):
		"""优化查询性能"""
		return super().get_queryset(request).select_related('meal', 'food', 'meal__user')


@admin.register(DailySummary)
class DailySummaryAdmin(admin.ModelAdmin):
	"""每日汇总管理界面"""
	list_display = ('user', 'date', 'get_total_calories', 'get_total_protein', 'get_total_fat', 'get_total_carbs', 'weight_recorded')
	...
	fieldsets = (
		...
		('营养汇总', {
			'fields': ('total_calories', 'total_protein', 'total_fat', 'total_carbs', 'total_fiber')
		}),
		('体重记录', {
			'fields': ('weight_recorded',)
		}),
		('时间戳', {
			'fields': ('created_at', 'updated_at'),
			'classes': ('collapse',)
