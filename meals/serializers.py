...
class MealFoodSerializer(serializers.ModelSerializer):
	...
	class Meta:
		model = MealFood
		fields = [
			'id', 'food', 'food_name', 'quantity', 'calories', 
			'protein', 'fat', 'carbs', 'food_calories_per_100g', 'added_at'
		]
...
class MealSerializer(serializers.ModelSerializer):
	...
	total_calories = serializers.DecimalField(max_digits=8, decimal_places=2, read_only=True)
	total_protein = serializers.DecimalField(max_digits=8, decimal_places=2, read_only=True)
	total_fat = serializers.DecimalField(max_digits=8, decimal_places=2, read_only=True)
	total_carbs = serializers.DecimalField(max_digits=8, decimal_places=2, read_only=True)
	
	class Meta:
		model = Meal
		fields = [
			'id', 'date', 'meal_type', 'name', 'notes', 
			'meal_foods', 'total_calories', 'total_protein', 
			'total_fat', 'total_carbs', 'created_at', 'updated_at'
		]
	...
	def validate_foods(self, value):
		...
		for food_item in value:
			if 'food_id' not in food_item:
				raise serializers.ValidationError("food_id is required for each food item")
...
class DailySummarySerializer(serializers.ModelSerializer):
	...
	class Meta:
		...
		fields = [
			...
			'total_fat', 'total_carbs', 'total_fiber', 
			'weight_recorded', 'created_at', 'updated_at'
		]
		read_only_fields = ['id', 'created_at', 'updated_at']
	...
	def validate_meals(self, value):
		...
		for meal in value:
			...
			if meal['meal_type'] not in valid_meal_types:
				raise serializers.ValidationError(f"Invalid meal_type: {meal['meal_type']}")
			if 'foods' not in meal:
				raise serializers.ValidationError("foods list is required for each meal")
