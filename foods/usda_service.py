...
class USDANutritionService:
	...
	def search_foods(self, query: str, page_size: int = 25, page_number: int = 1) -> Dict:
		...
		if not self.is_available():
			return {
				'success': False,
				'error': 'USDA API keys not configured'
			}
	...
	def get_food_details(self, fdc_id: int, nutrients: Optional[List[int]] = None) -> Dict:
		"""
	...
	def _extract_nutrition_from_search_result(self, food_item: Dict) -> Dict:
		...
		if not has_nutrition_data:
			logger.debug(f"No nutrition data found in search result for food: {food_item.get('description', 'Unknown')}")
