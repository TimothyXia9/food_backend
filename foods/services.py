...
class FoodDataService:
	...
	def create_custom_food(self, food_data: Dict[str, Any], user_id: int) -> Dict[str, Any]:
	...
	def update_food(self, food_id: int, food_data: Dict[str, Any], user_id: int) -> Dict[str, Any]:
		...
		try:
			...
			for field, value in food_data.items():
				if hasattr(food, field):
					if field in ['serving_size', 'calories_per_100g', 'protein_per_100g', 
								'fat_per_100g', 'carbs_per_100g', 'fiber_per_100g', 
								'sugar_per_100g', 'sodium_per_100g']:
	...
	def get_user_search_history(self, user_id: int, limit: int = 20) -> Dict[str, Any]:
		...
		try:
			searches = FoodSearchLog.objects.filter(user_id=user_id).order_by('-created_at')[:limit]
