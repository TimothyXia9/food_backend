...
class USDANutritionAPI:
	...
	def __init__(self):
		...
		if not api_key:
			...
			if not self.api_keys:
				raise ValueError("No USDA API keys found in environment variables (USDA_API_KEY or USDA_API_KEYS)")
		else:
	...
	def search_foods(self, query: str, page_size: int = 25, page_number: int = 1) -> Optional[Dict[str, Any]]:
		"""
	...
	def get_food_details(self, fdc_id: int, nutrients: Optional[List[int]] = None) -> Optional[Dict[str, Any]]:
		"""
...
def format_nutrition_info(food_data: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
