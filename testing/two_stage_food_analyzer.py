...
def load_config(config_path: str = None) -> Dict[str, Any]:
	...
	if config_path is None:
		config_path = Path(__file__).parent / "config_two_stage.json"
...
class FoodIdentificationAgent:
	...
	def __init__(self, config: Dict[str, Any], api_keys: List[str]):
		self.config = config
	...
	def _rotate_api_key(self):
		...
		if self.config["logging"]["enable_debug"]:
			print(f"🔄 FoodID Agent rotated to API key {self.current_key_index + 1}/{len(self.api_keys)}")
	...
	async def identify_foods_in_image(self, image_path: str) -> Dict[str, Any]:
		...
		user_message = {
			...
			"content": [
				{
					"type": "text",
					"text": "Please identify all foods in this image and estimate their portions. Do not search for nutrition data."
				},
				{
					"type": "image_url",
		...
		result = await self._call_openai_for_identification(
			messages, 
	...
	async def _call_openai_for_identification(
		self, 
		messages: List[Dict], 
		...
		"""Call OpenAI for food identification only"""
		...
		async with aiohttp.ClientSession() as session:
			for iteration in range(max_iterations):
				if self.config["logging"]["enable_debug"]:
					print(f"🔍 FoodID iteration {iteration + 1}/{max_iterations}")
				...
				try:
					...
					if self.config["logging"]["enable_debug"]:
						print(f"❌ FoodID API error: {e}")
					continue
			
			return {
				"success": False,
				"error": "Maximum iterations reached without successful response"
			}
...
class NutritionLookupAgent:
	...
	def __init__(self, config: Dict[str, Any], api_keys: List[str], agent_id: int = 0):
		self.config = config
		self.api_keys = api_keys
		self.current_key_index = agent_id % len(api_keys)  # Distribute keys among agents
	...
	def _rotate_api_key(self):
		self.current_key_index = (self.current_key_index + 1) % len(self.api_keys)
		if self.config["logging"]["enable_debug"]:
			print(f"🔄 Nutrition Agent #{self.agent_id} rotated to API key {self.current_key_index + 1}/{len(self.api_keys)}")
	...
	async def lookup_nutrition_for_food(self, food_item: Dict[str, Any]) -> Dict[str, Any]:
		...
		# Call OpenAI with function calling for nutrition lookup
		result = await self._call_openai_for_nutrition(
			messages,
			food_item,
			max_iterations=stage_config["max_iterations"],
			temperature=stage_config["temperature"],
		...
		if self.config["logging"]["show_timing"]:
			print(f"⏱️  Agent #{self.agent_id} nutrition lookup completed in {lookup_end - lookup_start:.2f} seconds")
	...
	async def _call_openai_for_nutrition(
		self,
		messages: List[Dict],
		...
		async with aiohttp.ClientSession() as session:
			for iteration in range(max_iterations):
				if self.config["logging"]["enable_debug"]:
					print(f"📊 Agent #{self.agent_id} nutrition iteration {iteration + 1}/{max_iterations}")
...
class TwoStageFoodAnalyzer:
	"""Coordinator for two-stage food analysis"""
	...
	def __init__(self, config_path: str = None):
		...
		self.food_identification_agent = FoodIdentificationAgent(self.config, self.api_keys)
		
		if self.config["logging"]["enable_debug"]:
			print(f"🤖 Two-Stage Food Analyzer initialized with {len(self.api_keys)} API key(s)")
	...
	async def _parallel_nutrition_lookup(
		...
		for i in range(max_concurrent):
			agent = NutritionLookupAgent(self.config, self.api_keys, agent_id=i)
...
async def test_two_stage_analysis(image_path: str, config_path: str = None):
	...
	print("🤖 Two-Stage Food Analysis Test")
	print("=" * 50)
