...
from calorie_tracker.two_stage_analyzer import TwoStageFoodAnalyzer, test_two_stage_analysis
...
class Command(BaseCommand):
	...
	def add_arguments(self, parser):
		...
		parser.add_argument(
			'--test-chat',
			action='store_true',
			help='Test basic chat completion'
		)
		parser.add_argument(
			'--test-vision',
			action='store_true',
			help='Test vision completion'
		)
		parser.add_argument(
			'--test-analyzer',
			action='store_true',
			help='Test two-stage food analyzer'
		)
		parser.add_argument(
			'--config',
			type=str,
	...
	def handle(self, *args, **options):
		...
		try:
			...
			self.stdout.write(self.style.ERROR(f"❌ Failed to initialize OpenAI service: {e}"))
			return
		...
		if options['test_analyzer'] and options['image']:
			...
			asyncio.run(self._test_two_stage_analyzer(options['image'], options.get('config')))
		
		# Auto-test if image is provided
		if options['image'] and not any([options['test_chat'], options['test_vision'], options['test_analyzer']]):
	...
	async def _test_chat_completion(self):
		...
		try:
			...
			if result['success']:
				...
				self.stdout.write(self.style.ERROR(f"❌ Chat completion failed: {result.get('error')}"))
	...
	async def _test_vision_completion(self, image_path):
		...
		try:
			...
			if result['success']:
				...
				self.stdout.write(self.style.ERROR(f"❌ Vision completion failed: {result.get('error')}"))
	...
	async def _test_two_stage_analyzer(self, image_path, config_path=None):
		...
		try:
			...
			if result and result.get('success'):
				...
				self.stdout.write(f"   • Foods identified: {summary['total_foods_identified']}")
				self.stdout.write(f"   • Successful lookups: {summary['successful_nutrition_lookups']}")
				self.stdout.write(f"   • Success rate: {summary['success_rate']}")
				self.stdout.write(f"   • Total calories: {summary['total_nutrition']['calories']}")
			else:
	...
	def _find_test_images(self):
		...
		test_image_dir = Path(__file__).parent.parent.parent.parent / "testing" / "test_images"
