...
class UploadedImageAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("基本信息", {"fields": ("user", "meal", "filename", "processing_status")}),
        (
            "图片信息",
            {
                "fields": (
        ...
        (
            "时间戳",
            {"fields": ("uploaded_at", "processed_at"), "classes": ("collapse",)},
        ),
    )
...
class FoodRecognitionResultAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("基本信息", {"fields": ("image", "food", "is_confirmed")}),
        (
            "识别结果",
            {
                "fields": (
    ...
    def get_queryset(self, request):
        """优化查询性能"""
        return (
            super().get_queryset(request).select_related("image", "food", "image__user")
        )
